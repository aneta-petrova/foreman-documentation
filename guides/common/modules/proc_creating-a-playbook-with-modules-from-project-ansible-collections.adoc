...
[NOTE]
====
To simplify vault file management, the following YAML snippets use module default groups by defining the `module_defaults` parameter.
With module default groups, any {ansible-namespace} module within the playbook automatically references the vault file, which eliminates the need to pass vault parameters to each module separately.
====
+
If you are authenticating with {Project} user credentials:
+
...
----
+
If you are authenticating with a Kerberos ticket or a Personal Access Token:
+
[source,yaml,subs="+quotes,attributes"]
----
- name: _My Playbook_
