...
The following example playbooks are executed against `localhost`, which can also be {ProjectServer} itself.
However, because Ansible modules from the {Project} Ansible Collections perform actions by using the {Project} API, you can execute your playbooks against any system that can reach the {Project} API.
