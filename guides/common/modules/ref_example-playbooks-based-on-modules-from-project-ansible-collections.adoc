...
The following example playbooks are executed on `localhost`, which can also be {ProjectServer} itself.
However, because Ansible modules from the {Project} Ansible Collections perform actions by using the {Project} API, you can run your playbooks on any system that can reach the {Project} API.
